...
- `Scripts/Flow/Video/Video - Cleaning Filters.js` - Adaptive filters: QSV `vpp_qsv=denoise` (merged into crop/format vpp when needed) + optional `deinterlace_qsv` (auto-detect via `idet`), plus optional CPU deband/gradfun and heuristic `mpdecimate` for Animation/Anime (duplicate-frame removal; forces VFR via `-vsync vfr`) via safe hwdownload/hwupload when enabled; also ensures the computed video filters are present in FFmpeg Builder "New mode" by updating `VideoStreams[0].EncodingParameters` (`-filter:v:0`) when required. Year fallback: extracts from filename (`.YYYY.` pattern, e.g., `Movie.1986.BluRay.mkv`) when `Variables.VideoMetadata.Year` is unavailable
- `Scripts/Flow/Video/Video - Auto Quality.js` - VMAF-based automatic quality detection (VMAF preferred; SSIM fallback) using binary search to find the highest compression that meets target quality (content-aware targets). Encodes test samples using the FFmpeg Builder encoder + `EncodingParameters` (e.g. QSV stays QSV; uses `-global_quality:v` for QSV) so results match the final pass, and extracts short video-only samples once to avoid repeatedly seeking the full file. If upstream filters require QSV, it pre-encodes high-quality filtered reference samples once; otherwise it compares against the raw/filtered source in the metric pass. Supports VMAF subsampling via `Variables.AutoQuality_VmafFps`; adds `-nostats` during metric runs to reduce log spam.
- `Scripts/Flow/Video/Video - Auto Tag Missing Language.js` - Detects missing/`und` (or force re-tags via `Variables['AudioLangID.ForceRetag']`) audio track languages using heuristics + offline LID (SpeechBrain; whisper.cpp fallback) and tags tracks via `mkvpropedit` (MKV) or ffmpeg remux; sample timing can be overridden via `Variables['AudioLangID.SampleStartSeconds']` / `Variables['AudioLangID.SampleDurationSeconds']`, duration fields are parsed from numeric seconds or `TimeSpan`-style strings, auto sampling avoids intros when duration is unknown, and the script updates in-memory `VideoInfo`/`FfmpegBuilderModel` audio `Language` properties so downstream nodes can read the new tags immediately (also stores `Variables['AudioLangID.UpdatedAudioLanguagesByIndex']`).
