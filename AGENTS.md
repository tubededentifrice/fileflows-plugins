...
- `Scripts/Flow/Video/Video - Language Based Track Selection.js` - Keeps only audio tracks matching the original language or specified additional languages; keeps unknown-language audio unless an original-language audio track exists; marks non-matching audio tracks with `Deleted=true`; never deletes subtitles (only reorders them using the same language priority rules); also syncs matching `VideoInfo` audio/subtitle arrays after reordering so downstream metadata readers see the output order; stores selection metadata in `Variables['TrackSelection.*']` for downstream scripts. Requires Movie/TV Show Lookup to run first for `Variables.OriginalLanguage`.
- `Scripts/Flow/Video/Video - Audio Format Converter.js` - Converts remaining (non-deleted) audio tracks to a target codec (eg `eac3`) with per-channel bitrate caps and an optional max sample-rate. Intended between Track Selection and the FFmpeg Builder Executor.
